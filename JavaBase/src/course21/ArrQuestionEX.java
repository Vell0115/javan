	...
	public static void main(String[] args) {
		...
		if (x == 4) {
			if (y == 2) {
				System.out.println("見たい月を入力してください。");
				int m = stdIn.nextInt();
				System.out.println(m +"月の台風・地震・降水量の総和は"+(typhoon[m-1]+quake[m-1]+rain[m-1])+"件です");
		...
		if (x == 4) {
			...
			int t = 0;
				for(int i =0; i < typhoon.length; i++) {
				for(int b =0; b < quake.length; b++) {
		        for(int p =0; p < rain.length; p++) {
		        s = s + typhoon[i];
		        l = l + quake[b];
		        t = t + rain[p];
		        }
				...
				System.out.println("一年間に起こった台風・地震・降水量の総和は" + s+"です。");
			}
	}
}
}
